...
class RotationTree:
    ...
    def __init__(self, dim_list, dim_used=None, new_dims=None, parent=None):
    ...
    def add_child(self, dim_list, dim_used=None, new_dims=None):

        child = RotationTree(dim_list,dim_used, new_dims, parent=self)
...
def rotate_tree(node,ALL_data,Angles):
    for child in node.children:
        if (child.new_dims==[]):
            ...
            for d in dim_used:  # Dimensions still not rotated
                for D in prev_dims: # Already rotated dimensions in previous node
                
                    for theta in Angles:  # NEW DIMS DEFINED BY ROTATING WITH CHOSEN ANGLES
                    
                        x_rot, y_rot = rotate(ALL_data[d], D, theta)
                        x_rot = (x_rot-x_rot.min())/x_rot.max()
                        y_rot = (y_rot-y_rot.min())/y_rot.max()
                        if np.isnan(x_rot.max()): print(x_rot)
...
def NaiveGMM(ALL_data,dim_plot=[],Rotation_Level=0,Angles=[np.pi/4]):
    ...
    if Rotation_Level > 0:
        ...
        for coup in couple_ind:  # All combinations of two dimensions to rotate for Level 1 rotations
            ...
            for theta in Angles:  # NEW DIMS DEFINED BY ROTATING WITH CHOSEN ANGLES
                ...
                new_dims.append(x_rot)
                new_dims.append(y_rot)
            rotation_root.add_child(dim_list=dim_list,new_dims=new_dims,dim_used=[couple_ind])
        ...
        for child in rotation_root.children:
            rotate_tree(child,ALL_data,Angles) # Generate new dimensions through rotations
        
        new_dims = assemble_dims(collect_dims(rotation_root)) # Collect all the new dimensions generated through rotations
                        
        ALL_data=np.concatenate((ALL_data,new_dims),axis=0)  # Add new dimensions to the data
    ...
    for i in range(n_dim): # Loop on all dimensions of original data + new dimensions obtained through rotations
        ...
        try:
            params, _ = curve_fit(double_gaussian, x, y, p0=p0,maxfev=10000,bounds=Bounds)
        ...
        try:
            params_alt, _ = curve_fit(single_gaussian, x, y, p0=p0_alt,maxfev=10000,bounds=Bounds)
            A1_alt, mu1_alt, sigma1_alt = params_alt
        
        except (ValueError, RuntimeError):
            warnings.warn(f"Bad fit in dimension {i} (1 gaussian): dimension ignored.")
            params_alt = p0_alt
            flag=1 #SECURITY
        ...
        bin_index = np.digitize(x_data,x_edges)
        ...
        if flag==1: # Ignore this dimension if one of the fits goes wrong
            ...
            ratio=chi(y,y_fit_alt,params_alt)/chi(y,y_fit,params) # Ratio used to estimate how useful a dimension is for the separation
            P_gauss1=g1[bin_index]/(g1[bin_index]+g2[bin_index]+1e-5)
    ...
    if len(PROBAS[PROBAS>0.5]) < len( (1-PROBAS)[(1-PROBAS)>0.5] ): PROBAS = 1-PROBAS # Order by size
